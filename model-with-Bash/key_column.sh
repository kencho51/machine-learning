...
for csv_file in `ls /processed-data/TAVG_US_*.csv`
do
    ...
    # > $csv_file.cut:
    cat $csv_file | cut -d',' -f1,2,4 > /processed-data/$csv_file.cut
    # mv $csv_file.cut $csv_file: Rename temporary file to original file
    mv /processed-data/$csv_file.cut /processed-data/$csv_file
